...
# 1 アップサンプリングの実行
# 2 アップサンプリングの結果検証
# 3 アップサンプリングのサンプル比率変更
# 4 ダウンサンプリングのサンプル比率変更
...
# ＜ポイント＞
# - SMOTEアルゴリズムによるアップサンプリングを行う
# - 少ない方のデータを指定した特徴量でKNNを用いてデータを増やしている


# レシピ作成
# --- 欠損値を平均で補完
# --- SMOTEアルゴリズムによるアップサンプリング
ds_rec <-
...
ds_rec %>% juice() %>% pull(Class) %>% table()


# 2 アップサンプリングの結果検証 ----------------------------------------

# ＜ポイント＞
# - アップサンプリングしたデータは元のデータセットの性質を保存している
#   --- データの形状や統計量は大きく変わらない
...
# ＜ポイント＞
# - アップサンプリングの各関数にはover_ratio引数が設定されている
...
# ＜ポイント＞
# - ダウンサンプリングの各関数には_ratio引数が設定されている
